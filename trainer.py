...
class Trainer(object):
    ...
    def _evaluate(self, output, label):
        output = self.torch.clamp(output, 0., 1.)
        labels = self.torch.clamp(label, 0., 1.)
        return [self.psnr(labels, output).item(), self.ssim(labels, output).item(), self.sam(labels, output).item()]

    def _output_save(self):
        with open(os.path.join(self.output_path, 'train_loss.pkl'), 'wb') as f:
            pickle.dump(self.train_output, f)
        with open(os.path.join(self.output_path, 'val_loss.pkl'), 'wb') as f:
